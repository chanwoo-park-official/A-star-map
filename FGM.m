...
theta(1) = (1 + sqrt(5))/2;
for k = 2:(N+3)
    theta(k) = (1 + sqrt(4* theta(k-1)^2 + 1))/2;
end
for k = 1:N
    if k ==1
        syk_real(1,1) = 1/L;
        szk_real(1,1) =(theta_zero)/L;
        sxk_real(1,1) = 1/theta(1) * szk_real(1,1)+ (theta(1) - 1 )/ theta(1) *syk_real(1,1);
    else
        syk_real(k,: ) = sxk_real(k-1,:);
        syk_real(k, k) = 1/L;
        szk_real(k, :) = szk_real(k-1, :);
        szk_real(k, k) = (theta(k-1))/L;
        sxk_real(k, :) = 1/theta(k) * szk_real(k,:)+ (theta(k) - 1) / theta(k) *syk_real(k,:);
    end
end
