...
for k = 1:(N-1)
    if k ==1
        ...
        szk_real(k, k) = k/L;
        sxk_real(k, :) = k/(k+2) * syk_real(k,:)+ 2 / (k+2) *szk_real(k,:);
    end
end

syk_real(N, :) = sxk_real(N-1, :);
syk_real(N, N) = 1/L;
szk_real(N,:) =  szk_real(N-1, :);
szk_real(N, N) = N/L;
sxk_real(N,:) = 1/(sqrt(N*(N+1)/2) + 1)* (sqrt(N*(N+1)/2) * syk_real(N,:) + szk_real(N, :));
